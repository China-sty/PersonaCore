...
> 当前进度：**Phase 1（文字）+ Phase 2（音频）已完成**，支持文字与语音面试。视频模态见 [`plan.md`](plan.md)。
...
- **语音面试（音频模态）**：ASR 转写（DashScope Paraformer）+ 情绪识别（emotion2vec），语音情绪作为辅助证据注入评分。
- **数据持久化 + 管理面板**：SQLite 自动落库，`/admin` 查看候选人列表、分数、雷达图与报告。
...
![面试界面](docs/images/interview.jpg)
